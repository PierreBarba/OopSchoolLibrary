...
require_relative 'modules'class App
  def initialize
    @books = []
    @users = []
    @rentals = []
    load_data
  end  include ExtraMethods  def books_list
    list = ''
    ...
    option = get_option_selected(1, 2)    case option
    when 1
      attach_student
    when 2
      attach_teacher
  ...
  end  def user_rentals
    print 'Input user id: '
    id = gets.chomp
    person = @users.find { |per| per.id == id }
    person.nil? ? "no valid id\n" : person.rental_list
  end  def get_option_selected(first, last)
    option = ''
    loop do
      option = gets.chomp.to_i
      break if option >= first && option <= last      print 'select a valid option '
    end
    option
  end  def permission?
    print 'has parent permission? [Y/N]'
    answer = gets.chomp.capitalize
    return true unless answer == 'n'    false
  end  def attach_student
